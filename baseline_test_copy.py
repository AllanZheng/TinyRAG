...
vector_path='vector/fine_tune_ocean' #/vector/kg_ocean/en ocean

# 创建EmbeddingModel
embedding = BgeEmbedding(path="C:\\Users\\Allan\\.cache\\huggingface\\hub\\new_epoch10")
# 创建RerankerModel
...
if have_created_db:
    ...
    vector.persist(path='vector/fine_tune_ocean')  # 将向量和文档内容保存到storage目录下，下次再用就可以直接加载本地的数据库
    time_3=time.time()
...
# try:
print("开始")
with open("result/test_fine_tune.json","w") as result_file,open("test_data/ocean/test20_base.json", 'r',encoding="utf-8") as que_file:
    count=0
    for line in que_file:
        data = json.loads(line)
        question = data['input']
        content = vector.query(question, EmbeddingModel=bgeEmbedding, k=5)
    ...
    result_file.write(str(result.ragas_traces))
