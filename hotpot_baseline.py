...
if have_created_db:
    ...
    docs = ReadFiles('./triple_data').get_content(max_token_len=600, cover_content=150)  # 获得data目录下的所有文件内容并分割
    time_1=time.time()
    ...
    vector.persist(path='vector/kg_ocean/en')  # 将向量和文档内容保存到storage目录下，下次再用就可以直接加载本地的数据库
    time_3=time.time()
...
# try:
with open("result/test2.txt","wb") as result_file,open("evaluate/ocean/test1.jsonl", 'r',encoding="utf-8") as que_file:
    
    for line in que_file:
        data = json.loads(line)
        question = data['en_input']
        ...
        dicts["reference"].append(data['en_output'])
    eval_data= Dataset.from_dict(dicts)
    result = Extra_evaluate(eval_data)
    print(result)
    df = result.to_pandas()
    df.to_csv("result/test1.csv",index=False)
    result_file.write(bytes(result.ragas_traces))
